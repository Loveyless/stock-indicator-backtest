...
策略作者只需要写一个文件：`strategy.js`，导出 `strategy(ctx)`，返回“本周期要买哪些股票（按 file）”。

> 安全提示：策略文件是本地 JS 代码，会在你的机器上执行，不要运行不可信的策略文件。
...
引擎当前是“理想化成交”口径（用于先把策略逻辑跑通与做相对比较）：
...
下面这份模板不追求花哨，只追求不容易写错（尤其是 `asOfYmd` 与 index 的取法）：
